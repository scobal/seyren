...
# [<img src="http://i.imgur.com/ahu3aM6.png" height="600" width="1120" />](http://i.imgur.com/ahu3aM6.png)
...
| `SEYREN_LOG_PATH` | The path of seyren.log. If a value is set, it must end with a '/'. Default:
...
| `GRAPHITE_CONNECTION_REQUEST_TIMEOUT`| The number of millisconds to wait to obtain a connection from the pool. Default: `0`
| `GRAPHITE_CONNECT_TIMEOUT`| The number of milliseconds to wait to establish a connection. Default: `0`
| `GRAPHITE_SOCKET_TIMEOUT`| The number of milliseconds to wait for request data. Default: `0`
...
| | |
| ----------------- |------------ |
| `PUSHOVER_APP_API_TOKEN`| Your pushover App API Token. Default:
...
To build Seyren and run the tests:

```
gradle clean build
```

To fire-up the app and wait (meaning you can run the tests separately from your IDE):

```
gradle clean build -Dwait
```

You should then be able to browse to `http://localhost:8080/seyren` and have a play.

You can use `docker-compose` to create containers and run Seyren.
* Install [Docker and Docker Compose](https://docs.docker.com/compose/#installation-and-set-up).
