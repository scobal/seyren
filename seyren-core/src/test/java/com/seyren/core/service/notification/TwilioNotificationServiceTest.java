...
public class TwilioNotificationServiceTest {
    ...
    public void verifyErrorAlert() {
       assertAlert(AlertType.ERROR);
    }

    @Test
    public void verifyWarningAlert() {
        assertAlert(AlertType.WARN);
    }

    private void assertAlert(AlertType alertType) {
        ...
        assertThat(body, containsString("Body=" + alertType.name() + "+Check+check-name+has+exceeded+its+threshold"));
